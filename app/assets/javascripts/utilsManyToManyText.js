...
var utilsManyToManyText = {};
(function (utilsManyToManyText, xrtc) {
	var _av = false,
		_room = null,
		_userName = null,
		_textChannel = [],
	...
	xrtc.Class.extend(utilsManyToManyText, {
		...
		connectionCreated: function(connectionData) {
			...
			utilsManyToManyText.subscribe( _connection, xrtc.Connection.events );
			...
			_connection
				...
				.on( xrtc.Connection.events.remoteStreamAdded, function (data) {
					...
					utilsManyToManyText.addVideo(data);
					utilsManyToManyText.refreshRoom();
				})
          ...
          var remoteUser = data.channel.getRemoteUser().name;
					...
					utilsManyToManyText.subscribe(_textChannel[remoteUser], xrtc.DataChannel.events);
					_textChannel[remoteUser].on( xrtc.DataChannel.events.sentMessage, function(msgData) {
						...
						utilsManyToManyText.addMessage(_textChannel[remoteUser].getRemoteUser().name, msgData.data);
					});
          
					utilsManyToManyText.addMessage("XirSys", "You connected with " + remoteUser + ".");
      ...
      utilsManyToManyText.addMessage(roomInfo.user.name, message, true);
      ...
      $chat[0].scrollTop = $chat[0].scrollHeight;
		...
		refreshRoom: function() {
			...
			var contacts = utilsManyToManyText.convertContacts(_room.getUsers());
			for (var index = 0, len = contacts.length; index < len; index++) {
				utilsManyToManyText.addUser(contacts[index]);
			}
		},
