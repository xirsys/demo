...
var utilsManyToManyVideoDynamic = {};
(function (utilsManyToManyVideoDynamic, xrtc) {
	var _av = false,
		_room = null,
		_userName = null,
		_textChannel = null,
	...
	xrtc.Class.extend(utilsManyToManyVideoDynamic, {
		...
		connectionCreated: function(connectionData) {
			...
			utilsManyToManyVideoDynamic.subscribe( _connection, xrtc.Connection.events );
			...
			_connection
				...
				.on( xrtc.Connection.events.remoteStreamAdded, function (data) {
					...
					utilsManyToManyVideoDynamic.addVideo(data);
					utilsManyToManyVideoDynamic.refreshRoom();
				})
				...
				.on( xrtc.Connection.events.dataChannelCreated, function (data) {
					...
					utilsManyToManyVideoDynamic.subscribe(_textChannel, xrtc.DataChannel.events);
					_textChannel.on( xrtc.DataChannel.events.message, function(msgData) {
						utilsManyToManyVideoDynamic.addMessage(msgData.userId, msgData.message);
					});
					utilsManyToManyVideoDynamic.addMessage("SYSTEM", "You are now connected.");
      ...
      if (data.isLocalStream) {
		...
		sendMessage: function (message) {
			...
			if (_textChannel) {
				...
				utilsManyToManyVideoDynamic.addMessage( _userName, message, true );
			} else {
		...
		refreshRoom: function() {
			...
			var contacts = utilsManyToManyVideoDynamic.convertContacts(_room.getUsers());
			for (var index = 0, len = contacts.length; index < len; index++) {
				utilsManyToManyVideoDynamic.addParticipant(contacts[index]);
			}
		},
...
})(utilsManyToManyVideoDynamic, xRtc);