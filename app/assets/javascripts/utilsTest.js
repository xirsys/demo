...
(function (utilsTest, xrtc) {
    ...
    // Set middle tier service proxies (on server)
	  // This is the server page which handles the calls 
		...
		connectionCreated: function(connectionData) {
			...
			utilsTest.subscribe( _connection, xrtc.Connection.events );
			...
			_connection
				...
				.on( xrtc.Connection.events.remoteStreamAdded, function (data) {
					...
					utilsTest.addVideo(data);
					utilsTest.refreshRoom();
				})
        ...
        // _textChannel is an array to handle multiple users
          ...
          var remoteUser = data.channel.getRemoteUser().name;
					...
					utilsTest.subscribe(_textChannel[remoteUser], xrtc.DataChannel.events);
					_textChannel[remoteUser].on( xrtc.DataChannel.events.sentMessage, function(msgData) {
						...
						utilsTest.addMessage(_textChannel[remoteUser].getRemoteUser().name, msgData.data);
					});
          
					utilsTest.addMessage("XirSys", "You connected with " + remoteUser + ".");
        ...
        // You may wish to add real functionality here
          ...
          utilsTest.addMessage("XirSys", "You disconnected from " + data.user.name + ".");
      ...
      utilsTest.addMessage(roomInfo.user.name, message, true);
      ...
      if (textChannelSize == 0) {
        utilsTest.addMessage("XirSys", "No one can hear you right now.");
      } else {
    ...
    // Adds a message to local chat box
      ...
      // $chat[0].scrollTop = $chat[0].scrollHeight;
		...
		refreshRoom: function() {
			...
			var contacts = utilsTest.convertContacts(_room.getUsers());
			for (var index = 0, len = contacts.length; index < len; index++) {
				utilsTest.addUser(contacts[index]);
			}
		},
    ...
    // Only connects if there isn't already an existing connection with participant
      ...
      if (!_textChannel[participant]) {
        utilsTest.room().connect(participant, { createDataChannel: 'auto' });
      }
    },
    
    // Only connects if there isn't already an existing connection with participant
		enterRoom: function(participant) {
      utilsTest.addMessage("XirSys", "You Joined the room as " + _userName + ".");
    }
